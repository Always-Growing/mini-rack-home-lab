...
- **Native Sink (VLAN 999):** A **Native Sink** (VLAN 999) is used on exposed trunk ports to neutralize untagged traffic. This serves as a safeguard, providing an early warning of unauthorized access attempts and ensuring that untagged traffic is discarded. For simplicity in this home lab, unused ports can be tagged as VLAN 30, so any unauthorized access would only provide basic internet connectivity rather than risking other parts of the network.
- **Tape Library for Backups:** While **enterprise tape backup systems** would be used in a larger network for data redundancy, due to space and cost restraints, this setup does not include tape libraries. Backup in this lab is handled by **a dedicated backup NAS** (TrueNAS) and **Proxmox clusters** to support high availability. In the future, I can explore offsite storage options to follow the 3-2-1 rule for data protection.
...
| **70** | STAGING | 10.100.70.0/24 | **Yes** | **Yes** | Setup VLAN for new VMs and containers which need internet access for setup. |
| **98** | Out of Band Management | 10.100.98.0/24 | **Yes** | **Yes** | Out of Band Management for Lenovo M90Q Tiny units. These nodes will be assigned static IPs via a limited DHCP pool |
